from .models import GCN, GAT, GIN, RGCN, RGAT, CompGCN, HGT, HAN, GCNDelete, GATDelete, GINDelete, RGCNDelete, RGATDelete, HGTDelete, HANDelete
...
try:
    ...
    MIAttackTrainerNode = MIAttackTrainer

try:
    from .trainer.graph_editor import GraphEditorTrainer
except ImportError:
    GraphEditorTrainer = None
...
if GraphEditorTrainer is not None:
    trainer_mapping['graph_editor'] = GraphEditorTrainer


def get_model(args, mask_1hop=None, mask_2hop=None, num_nodes=None, num_edge_type=None):

    if 'gnndelete' in args.unlearning_model:
        model_mapping = {'gcn': GCNDelete, 'gat': GATDelete, 'gin': GINDelete, 'rgcn': RGCNDelete, 'rgat': RGATDelete, 'hgt': HGTDelete, 'han': HANDelete}

    else:
        model_mapping = {'gcn': GCN, 'gat': GAT, 'gin': GIN, 'rgcn': RGCN, 'rgat': RGAT, 'compgcn': CompGCN, 'hgt': HGT, 'han': HAN}
