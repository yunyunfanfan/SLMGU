...
def main():
    args = parse_args()
    base_checkpoint_dir = 'checkpoint_node'
    args.checkpoint_dir = 'checkpoint_node_feature'
    args.dataset = 'DBLP'
    original_path = os.path.join(base_checkpoint_dir, args.dataset, args.gnn, 'original', str(args.random_seed))
    attack_path_all = os.path.join(base_checkpoint_dir, args.dataset, 'member_infer_all', str(args.random_seed))
    attack_path_sub = os.path.join(base_checkpoint_dir, args.dataset, 'member_infer_sub', str(args.random_seed))
    seed_everything(args.random_seed)

    if 'gnndelete' in args.unlearning_model:
        args.checkpoint_dir = os.path.join(
            args.checkpoint_dir, args.dataset, args.gnn, f'{args.unlearning_model}-node_feature_deletion', 
    ...
    if args.df_size >= 100:     # df_size is number of nodes/edges to be deleted
        ...
        df_size = max(1, int(args.df_size / 100 * data.num_nodes))
    df_size = min(df_size, data.num_nodes)
    ...
    if args.unlearning_model != 'retrain':
        retrain_path = os.path.join(
            'checkpoint_node', args.dataset, args.gnn, 'retrain', 
            '-'.join([str(i) for i in [args.df, args.df_size, args.random_seed]]))
        retrain_file = os.path.join(retrain_path, 'model_best.pt')
        if os.path.exists(retrain_file):
            retrain_ckpt = torch.load(retrain_file, map_location=device)
